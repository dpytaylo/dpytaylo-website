...
impl Texture {
    // pub async fn new(gl: WebGl2RenderingContext, extensions: &Extensions, path: &str, use_near_filter: bool) 
    ...
    //     log::info!("Texture({path}) is {:?}", output_info.color_type);
    ...
    //             log::info!("bit_depth = {bit_depth}");
    //             log::info!("info.bits_per_pixel() / bit_depth = {}", info.bits_per_pixel() / bit_depth);
    //             log::info!("pallete.len() % 4 = {}", pallete.len() % 4);
